(function () {
    ...
    let thumbnails = Array.from(document.querySelectorAll('.product__thumbnail'));
    let currentThumbnail = 0; // first image is shown on page load

    function findCurrentThumbnail(img) {
        return thumbnails.indexOf(img);
        // let foundIdx = -1;
        ...
        // // for loop solution
        // for (let idx = 0; idx < thumbnails.length; idx += 1) {
        //     if (thumbnails[idx] == img) {
        //         foundIdx = idx;
        //         idx = thumbnails.length; // end loop
        //     }
        // }
    ...
    document.querySelector('.product__thumbnails').addEventListener('click', (evt) => {
        ...
        if (evt.target.className == 'product__thumbnail') {
            ...
            currentThumbnail = findCurrentThumbnail(evt.target);
        }
    });
