...
/*
    ...
    2. Create a renderBooks method which renders the books into the table
    ...
    3. Select the table body, select the form and call the two functions from the
    previous steps. This should show in the table
    4. Create a getAuthors function
      - use map to create an array of only authors
      - use filter to create an array of only distinct authors
      - sort as a function on an array to show you can sort things
    5. Create the function renderAuthorOptions to add the options to the select

    6. Add the submit event listener to the form
    7. Get the necessary form values
...
let allBooks = getAllBooks(); // simulate a call to a backend serivce

renderBooks(allBooks); // render all when the page loads
renderAuthorOptions(allBooks);

filterForm.addEventListener('submit', (evt) => {
  evt.preventDefault();
  let title = filterForm.elements['filter-query'].value.trim();
  let author = filterForm.elements.author.value;
  let theBooks = allBooks;

  if (title !== '') {
    theBooks = filterBooks(title, 'title', theBooks);
  }

  if (author !== '') {
    theBooks = filterBooks(author, 'author', theBooks);
  }

  renderBooks(theBooks);
});

function filterBooks(filterQuery, bookProperty, books) {
...
function renderAuthorOptions(books) {
  ...
  // render an option for each author in the array
...
function getAuthors(books) {
  ...
  let distinct = authors.filter((author, idx) => {
